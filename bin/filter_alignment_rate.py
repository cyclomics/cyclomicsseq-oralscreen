...
def get_amplicon_lengths(in_file: str) -> Dict[str, int]:
    ...
    Args:
        in_file (FilePath): Path to input TSV file
            (column 1: amplicon name; column 2: expected amplicon length)
    ...
    with open(in_file, "r") as file:
        for line in file:
            amplicon, length = line.strip().split("\t")
            amplicon_lengths[amplicon] = int(length)
...
if __name__ == "__main__":
    ...
    if not dev:
        ...
        filter_alignment_rate(
            args.input_bam, args.output_bam, args.amplicons_tsv, args.min_align_rate
        )

    else:
        input_bam = "PAW56644.YM_gt_3.bam"
        output_bam = "test.bam"
        amplicons_tsv = "bin/variant_calling/amplicon_lengths.tsv"
        min_align_rate = 0.8

        filter_alignment_rate(input_bam, output_bam, amplicons_tsv, min_align_rate)
